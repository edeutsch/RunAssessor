...
class MzMLAssessor:
    ...
    def find_ms_one_tolerance(self, ms_one_tolerance_dict):
        ...
        try:
            ...
            self.precursor_stats['precursor tolerance']['fit_ppm'] = {"lower_three_sigma (ppm)":-fit_ppm[2]*3, "upper_three_sigma (ppm)":fit_ppm[2]*3, "delta_ppm peak": fit_ppm[1], "intensity": fit_ppm[0], "sigma (ppm)": fit_ppm[2], "y_offset": fit_ppm[3], "chi_squared":chi_squared_red_ppm, "recommended precursor tolerance (ppm)": f"{math.ceil(math.sqrt(5**2 + (fit_ppm[2]*3)**2))} ppm"}
            ...
            if (self.all_3sigma_values_away['Status']):
                ...
                try:
                    if fragmentations.startswith('HR'):
                        ...
                        recommended_precursor = math.ceil(math.sqrt(5**2 + (max(abs(three_sigma_lower), abs(three_sigma_upper))**2)))
                        ...
                        recommended_precursor = round(math.sqrt(0.3**2 + (max(abs(three_sigma_lower), abs(three_sigma_upper)))**2), 3)
